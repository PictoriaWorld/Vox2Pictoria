...
public class ParsedVox(byte[] fullFileBytes, int version)
{
    ...
    // .vox format version (e.g. 150 = MagicaVoxel 0.99.7.2+)
    public int Version { get; } = version;

    // SIZE+XYZI pairs in order (model index = list index)
    public List<(RawVoxChunk SizeChunk, RawVoxChunk XyziChunk)> ModelChunks { get; } = new();

    // Scene graph: all nTRN, nGRP, nSHP chunks (raw content bytes)
    public List<RawVoxChunk> SceneGraphChunks { get; } = new();
    ...
    public List<Memory<byte>> MatlChunks { get; } = new();
}